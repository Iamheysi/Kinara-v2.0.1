...
export function ProfileTab({c,t,lang,sessions,restDaysLog,plans,profileName,setProfileName,profileBio,setProfileBio,profileGoal,setProfileGoal,profilePhoto,setProfilePhoto,photoInputRef,showToast,achievements,onOpenAchievements}){
  ...
  // Show first 8 achievements in preview
  const achPreview=achList.slice(0,8);
  ...
  return(<div style={{maxWidth:1100,margin:"0 auto"}}>
    ...
    {showMiniProfile&&(<div onClick={()=>setShowMiniProfile(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:500,backdropFilter:"blur(3px)"}}>
      <div onClick={e=>e.stopPropagation()} style={{position:"absolute",top:80,left:32,background:c.card,border:`1px solid ${c.borderMid}`,borderRadius:16,padding:"20px 22px",width:280,boxShadow:"0 12px 40px rgba(0,0,0,0.25)",animation:"fadeUp .2s ease"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:5}}>
          ...
          <button onClick={()=>{setShowMiniProfile(false);if(onOpenAchievements)onOpenAchievements();}} style={{width:"100%",background:c.surface,border:`1px solid ${c.border}`,borderRadius:8,padding:"8px 12px",fontSize:12,color:c.textPrimary,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",textAlign:"left",display:"flex",alignItems:"center",gap:8}}>{Ic.trophy} {t.achievements}</button>
        </div>
      </div>
    </div>)}

    {/* Profile Header Card */}
    <div className="kb-prof-top" onClick={()=>!editing&&setShowMiniProfile(true)} style={{background:`linear-gradient(135deg,${c.card},${c.surface})`,border:`1px solid ${c.border}`,borderRadius:20,padding:"24px 28px",marginBottom:18,display:"grid",gridTemplateColumns:"auto 1fr auto",gap:24,alignItems:"start",boxShadow:`0 4px 20px rgba(0,0,0,0.08), inset 0 1px 0 ${c.white}22`,cursor:editing?"default":"pointer",transition:"box-shadow 0.2s"}}>
      <div style={{position:"relative"}} onClick={e=>e.stopPropagation()}><div style={{width:88,height:88,borderRadius:22,background:profilePhoto?`url(${profilePhoto}) center/cover`:`linear-gradient(135deg,${c.primary},${c.primaryLight})`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Barlow Condensed',sans-serif",fontSize:30,fontWeight:900,color:"#fff",border:`3px solid ${c.border}`,boxShadow:`0 4px 16px ${c.primary}33`}}>{!profilePhoto&&(initials||<span style={{opacity:0.7}}>{Ic.user}</span>)}</div><input ref={photoInputRef} type="file" accept="image/*" onChange={handlePhotoPreview} style={{display:"none"}}/><button onClick={()=>photoInputRef.current?.click()} style={{position:"absolute",bottom:-6,right:-6,width:28,height:28,borderRadius:"50%",background:c.primary,border:`2px solid ${c.surface}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"#fff"}}>{Ic.camera}</button></div>
    ...
    {/* Achievements Card — clickable to open full modal */}
    <div onClick={()=>onOpenAchievements&&onOpenAchievements()} style={{background:`linear-gradient(135deg,${c.card},${c.primaryDim})`,border:`1px solid ${c.primary}22`,borderRadius:16,padding:"18px 22px",marginBottom:14,cursor:"pointer",transition:"all 0.2s",boxShadow:`0 2px 12px ${c.primary}11`}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <span style={{fontSize:18}}>{Ic.trophy}</span>
          <p style={{fontSize:14,fontWeight:700,color:c.textPrimary,fontFamily:"'Barlow Condensed',sans-serif"}}>{t.achievements}</p>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <span style={{fontSize:12,fontWeight:600,color:c.primary,background:c.primaryDim,padding:"3px 10px",borderRadius:12,fontFamily:"'JetBrains Mono',monospace"}}>{earned.length}/{achList.length}</span>
          <span style={{color:c.textMuted}}>{Ic.arrow}</span>
        </div>
      </div>
      ...
      <div style={{height:4,background:c.border,borderRadius:2,overflow:"hidden"}}>
        <div style={{height:"100%",width:`${achList.length>0?(earned.length/achList.length)*100:0}%`,background:`linear-gradient(to right,${c.primary},${c.primaryLight})`,borderRadius:2,transition:"width 0.5s"}}/>
      </div>
      <p style={{fontSize:10,color:c.textMuted,marginTop:6,textAlign:"right"}}>{isRu?"Нажмите для подробностей":"Tap to view all"} →</p>
    </div>
